...
N_s = 64;
N_T = 1e3*N_mu_true;
a_kt_true = 0*(2*rand(N_mu_true,N_T) - 1); % uniform in [-1,1]
L_t_true = [];
time_index=1;
for k = 1:N_mu_true
  L_t_true = [L_t_true, k*ones(1,N_T/N_mu_true)];
  a_kt_true(k,time_index:time_index +N_T/N_mu_true-1 ) =2*rand(1,N_T/N_mu_true) - 1;
  time_index = time_index + N_T/N_mu_true;
end

function [ average_lifespan,frequency,coverage,amplitude,transition_matrix,GEV ] = u_state_statistics( u_state_sequence,sampling_time,GFP )
...
u_states = unique(u_state_sequence); %vector of integers
N_u = length(u_states); 
...
amplitude(prev_u_state) = amplitude(prev_u_state) +  GFP(1);
for t = 1:N_T-1
   next_u_state = u_state_sequence(t+1);
   amplitude(next_u_state) = amplitude(next_u_state) + GFP(t+1);
